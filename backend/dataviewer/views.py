...
@api_view(['GET'])
def get_metadata_deidentified(request):
    """비식별화 컬럼 메타데이터 조회 API"""
    try:
        metadata = ExcelDataService.get_metadata(deidentified=True)
        return Response({
            'success': True,
            'data': metadata
        })
...
@api_view(['GET'])
def get_data_deidentified(request):
    """비식별화 데이터 조회 API (페이징 및 검색)"""
    try:
        page = int(request.GET.get('page', 1))
        page_size = int(request.GET.get('page_size', 50))
        patient_no = request.GET.get('patient_no', '').strip()
...
@api_view(['POST'])
def update_data_deidentified(request):
    """비식별화 데이터 업데이트 API"""
    try:
        row_index = request.data.get('row_index')
        column_name = request.data.get('column_name')
        ...
        ExcelDataService.update_data(row_index, column_name, value, deidentified=True)
...
@api_view(['POST'])
def update_data_deidentified_by_patient(request):
    """환자번호 + 중재일자로 비식별화 데이터 업데이트 API"""
    try:
        ...
        if not patient_no or not intervention_date or not column_name:
            return Response({
                'success': False,
                'error': 'patient_no, intervention_date, column_name이 필요합니다.'
            }, status=status.HTTP_400_BAD_REQUEST)

        ExcelDataService.update_data_by_patient(patient_no, intervention_date, column_name, value, deidentified=True)
